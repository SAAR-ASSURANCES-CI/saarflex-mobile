...
class _SimulationResultScreenState extends State<SimulationResultScreen> {
  ...
  void _handleSaveMessages(SimulationViewModel provider) {
  ...
  Widget _buildSaveSection(SimulationViewModel provider) {
  ...
  void _handleSaveQuote(SimulationViewModel provider) async {
    if (widget.resultat.statut == StatutDevis.sauvegarde) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
  ...
  void _showSaveConfirmationDialog(SimulationViewModel provider) {
    showDialog(
      context: context,
